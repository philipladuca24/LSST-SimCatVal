...
dataset = oc.open(diffsky_path, synth_cores=True)
...
dec = np.linspace(mindec, maxdec, point_count)
ra = np.linspace(minra, maxra, point_count*np.cos(avgdec))
...
diffsky_paths = glob('/hildafs/home/pladuca/share/cosmos_260120_UM_2026_01_21/lc_cores*.hdf5')
ds_list = []
for diff in diffsky_paths:
    ...
    bound_ds = bound_ds.select(['lsst_u', 'lsst_u_bulge','lsst_u_knots','lsst_u_disk',
                            ...
                            'ra','dec','redshift','logsm_obs',
                            'beta_bulge','alpha_bulge','ellipticity_bulge','psi_bulge',
                            'ellipticity_disk','beta_disk','alpha_disk','psi_disk', 'r50_disk', 'r50_bulge'])
...
#figure out how to set up saving
